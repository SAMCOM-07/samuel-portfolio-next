...
const Faqs = () => {
  ...
  return (
    <section className='p-6 max-w-xl mx-auto mb-16' aria-labelledby="faq-heading">
      <h1 id="faq-heading" className='mb-8 text-center'>Frequently Asked Questions</h1>
      <div className='space-y-4' role="list">
        {faqs.map((faq) => (
          <Card key={faq.id}>
            <div className='p-2 transition-all duration-500 overflow-hidden' role="listitem">
              <button
                onClick={() => setSelected(selected === faq.id ? null : faq.id)}
                aria-expanded={selected === faq.id}
                aria-controls={`faq-answer-${faq.id}`}
                type="button"
                className='w-full flex items-start justify-between gap-6 cursor-pointer outline-0 focus:outline-none focus:ring-2 focus:ring-primary rounded transition-colors'
              >
