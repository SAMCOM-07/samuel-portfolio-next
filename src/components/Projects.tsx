...
const Projects = () => {
  ...
  return (
    <div className="w-full relative mt-22 pt-16 pb-6 conpad">
      {/* background */}
      ...
      <div className='flex flex-col gap-32 container mx-auto mt-16 pb-6'>
        {
          projectsDataSlice && projectsDataSlice.map((project) =>
            < div key={project.id} className={`flex flex-col ${project.id % 2 === 0 ? 'lg:flex-row-reverse' : 'lg:flex-row'} items-start gap-6 lg:gap-12 border-2 border-border rounded-lg p-4`}>
              {/* image */}
